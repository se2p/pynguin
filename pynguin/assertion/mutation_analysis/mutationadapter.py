...
class MutationAdapter:
    ...
    def mutate_module(self) -> List[Tuple[ModuleType, List[mo.Mutation]]]:
        """"Mutates the modules specified in the configuration by using MutPys' mutation
        procedure.
        ...
        for target_module, to_mutate in self.target_loader.load():
            ...
            mutant_modules = controller.mutate_module(target_module=target_module,
                                                      to_mutate=to_mutate,
    ...
    def _build_mutation_controller(self) -> mc.MutationController:
        ...
        self.target_loader = mu.ModulesLoader([config.configuration.module_name],
                                              config.configuration.project_path)

        return mc.MutationController(runner_cls=None,
