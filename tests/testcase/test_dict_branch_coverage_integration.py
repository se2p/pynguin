...
def test_dict_none_branch_execution_and_collection_trace(subject_properties: SubjectProperties):
    module_name = "tests.fixtures.branchcoverage.dict_branch"
    config.configuration.module_name = module_name
    with (
        install_import_hook(
    ...
    # Statement 1 raised KeyError because 'null_key' is missing from var_0
    assert 1 in result.exceptions
    assert isinstance(result.exceptions[1], KeyError)
    assert result.exceptions[1].args[0] == "null_key"

    assert 0 in result.collection_trace
    assert "null_key" in result.collection_trace[0].missing_keys
