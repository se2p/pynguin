...
from pynguin.ga.algorithms.archive import (
    CoverageArchive,
    MIOArchive,
    MIOPopulation,
    MIOPopulationPair,
...
def test_coverage_archive_protected_objective_keeps_incumbent():
    """A protected objective's incumbent must survive even a "better" candidate.

    Regression test for the archive-cannibalization bug: an initial random
    population used to immediately evict a richer LLM-seeded solution in favor
    of a trivial shorter one covering the same objective.
    """
    objective = MagicMock(ff.TestCaseFitnessFunction)
    archive = CoverageArchive(OrderedSet([objective]))
    ...
    seeded_llm_solution = MagicMock(tcc.TestCaseChromosome)
    seeded_llm_solution.size.return_value = 5
    seeded_llm_solution.get_is_covered.return_value = True
    seeded_llm_solution.get_last_execution_result.return_value = ok_result

    trivial_random_solution = MagicMock(tcc.TestCaseChromosome)
    trivial_random_solution.size.return_value = 1
    trivial_random_solution.get_is_covered.return_value = True
    trivial_random_solution.get_last_execution_result.return_value = ok_result

    archive.update([seeded_llm_solution])
    assert archive._covered[objective] == seeded_llm_solution

    # Without protection, the shorter random solution would normally win.
    updated = archive.update([trivial_random_solution], protected=OrderedSet([objective]))

    assert updated is False
    assert archive._covered[objective] == seeded_llm_solution


def test_coverage_archive_unprotected_objective_still_updates():
    """Objectives not in ``protected`` are updated as usual."""
    protected_objective = MagicMock(ff.TestCaseFitnessFunction)
    other_objective = MagicMock(ff.TestCaseFitnessFunction)
    archive = CoverageArchive(OrderedSet([protected_objective, other_objective]))

    solution = MagicMock(tcc.TestCaseChromosome)
    solution.size.return_value = 1
    solution.get_is_covered.return_value = True

    updated = archive.update([solution], protected=OrderedSet([protected_objective]))

    assert updated is True
    assert other_objective in archive._covered
    assert protected_objective not in archive._covered


def test_mio_archive_protected_target_is_skipped():
    """A protected target in ``MIOArchive`` must not be touched by an update."""
    fitness = MagicMock()
    ...
    updated = archive.update([solution], protected=OrderedSet([fitness]))
